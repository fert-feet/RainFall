...
spec_train_features_path = f'./data/{config.NAME_SPEC_TRAIN_FEATURES_FILE}.npy'
spec_test_features_path = f'./data/{config.NAME_SPEC_TEST_FEATURES_FILE}.npy'
...
spec_train_dataset = USRADataset(train_labels_path, spec_train_features_path)
spec_val_dataset = USRADataset(test_labels_path, spec_test_features_path)
...
mfcc_train_loader = DataLoader(mfcc_train_dataset, shuffle=True, batch_size=batch_size,
                              collate_fn=USRADataset_collate)
spec_train_loader = DataLoader(spec_train_dataset, shuffle=True, batch_size=batch_size,
                              collate_fn=USRADataset_collate)
...
mfcc_test_loader = DataLoader(mfcc_val_dataset, shuffle=False, batch_size=batch_size,
                         collate_fn=USRADataset_collate)
spec_test_loader = DataLoader(spec_val_dataset, shuffle=False, batch_size=batch_size,
                         collate_fn=USRADataset_collate)
...
MSE_list = []
RMSE_list = []
total_step = len(mfcc_train_loader)
# # ---------------------------------------#
...
for epoch in range(num_epochs):
    ...
    for i, (mfcc_data, spec_data) in enumerate(zip(mfcc_train_loader, spec_train_loader)):
        ...
        # Forward pass
        rainfall_intensity = model(mfcc_images, spec_images)
    ...
    spec_acoustic_feature = spec_val_dataset.feature
    outputs = []
    step = 16
    with torch.no_grad():
        ...
        for index in tqdm(range(0,mfcc_acoustic_feature.shape[0],step)):
            if index ==0:
                ...
                step_spec_acoustic_feature = spec_acoustic_feature[index:step].to(torch.float)
                rainfall_intensity = model(step_mfcc_acoustic_feature, step_spec_acoustic_feature)
                ...
                step_spec_acoustic_feature = spec_acoustic_feature[index:spec_acoustic_feature.shape[0]].to(torch.float)
                rainfall_intensity = model(step_mfcc_acoustic_feature, step_spec_acoustic_feature)
                outputs = torch.cat((outputs, rainfall_intensity))
